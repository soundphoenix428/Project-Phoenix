...
import { WarnCommand } from './WarnCommand'; // Import the WarnCommand
...
export class InfoCommand implements Command {
    ...
    public async execute(intr: ChatInputCommandInteraction, data: EventData): Promise<void> {
        ...
        switch (args.option) {
            ...
            case InfoOption.BAN: {
                // Use the BanCommand for the ban functionality
                const banCommand = new BanCommand();
                await banCommand.execute(intr, data);
                return;
            }
            case InfoOption.WARN: {
                // Use the WarnCommand for the warn functionality
                const warnCommand = new WarnCommand();
                await warnCommand.execute(intr, data);
                return;
            }
            default: {
                return;
